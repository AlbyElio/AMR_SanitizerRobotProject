...
class BasicNavigator(Node):
    def __init__(self):
        ...
        self.ok_subscriber = self.create_subscription(Bool, 'ok_topic', self.ok_callback, qos_profile)
    ...
    def energy_map_callback(self, msg):
        if self.initial_pose_received :
            ...
            #self.info(f'!!!!!!!! RISOLUZIONE: {msg.info.resolution}')
...
def main(argv=sys.argv[1:]):
    ...
    while not navigator.localization_ok or navigator.energy_matrix._info.resolution == 0:
    ...
    for room in selected_rooms:
        ...
        # Bathroom 2
            ...
            y_room_range.extend(np.arange(-4.6, -0.6, res))
        # Kitchen 2
        elif room == 3:
            x_room_range.extend(np.arange(-4.6, 4.4, res))
        ...
        # Living room
            ...
            y_room_range.extend(np.arange(-7.6, -4.4, res))
            y_room_range.extend(np.arange(-7.6, -5.6, res))
            y_room_remove_range.extend(np.arange(-4.6, -3.4, res))
            y_room_remove_range.extend(np.arange(-7.6, -7.2, res))
            y_room_remove_range.extend(np.arange(-4.6, -3.4, res))
            y_room_remove_range.extend(np.arange(-4.6, -3.4, res))
        # Office
        elif room == 5:
